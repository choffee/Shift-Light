...
// colour - (byte array of three values defing an RGB colour to display
// (colour[0] = new Red value, colour[1] = new Green value, colour[2] = new Red value
void Arduino::Lights::TriColour::setColour(byte* colour )
{
  for(int i = 0; i < 3; i++)
  {
    ...
    //set the analog output value of each pin to the input value 
    //(ie led[0] (red pin) to 255- colour[0] (red input colour)
    //we use 255 - the value because our RGB LED is common anode, 
    //this means a colour is full on when we output analogWrite(pin, 0)
